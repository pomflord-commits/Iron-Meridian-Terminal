...
const ChatItem = ({ chat, active, onClick }) => (
  <div className={`chat-item ${active?"chat-item--active":""}`} onClick={onClick}>
    ...
    <div className="chat-item-meta"><span className="chat-item-preview">{chat.preview}</span><span className="chat-item-time">{chat.time}</span></div>
  </div>
);
...
const DEFAULT_CHAT_SETTINGS = {
  ...
  blueprintMode: "AUTO",       // AUTO | LOCKED | CUSTOM
...
export default function IronMeridianTerminal() {
  ...
  const generateBlueprint = async () => {
    if(isGeneratingBp || ollamaStatus!=="RUNNING") return;
...
Choose the type that best matches the conversation topic. Extract real details from the conversation. Keep names and values concise.`;
        ...
        if(jsonMatch){
          ...
          if(bp.type && RENDERERS[bp.type]){
            ...
            setChats(prev => prev.map(c => c.id!==activeChatId?c:{
              ...c, blueprintMode:"CUSTOM", customBlueprint:merged, blueprintView:"front"
            }));
          }
        }
      }
    } catch(e){ console.error("Blueprint generation failed:",e); }
    finally { setIsGeneratingBp(false); }
  };
  ...
  const resetBlueprint = () => {
    setChats(prev=>prev.map(c=>c.id!==activeChatId?c:{
      ...c, blueprintMode:"AUTO", customBlueprint:null, lockedBlueprint:null, blueprintView:"front"
    }));
  };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .cmd-input{flex:1;background:transparent;border:none;color:var(--text);font-family:var(--mono);font-size:12px;padding:11px 8px 11px 0;resize:none;outline:none;letter-spacing:.04em;min-height:44px;max-height:120px;scrollbar-width:none;line-height:1.4;}
        ...
        /* ── Status Bar ── */
        .statusbar{grid-column:1/-1;display:flex;align-items:center;justify-content:space-between;padding:0 16px;border-top:1px solid var(--border-accent);background:var(--panel);font-size:8px;color:var(--text-dim);letter-spacing:.12em;}
        ...
        <aside className="sidebar-right">
          ...
          <div className="bp-actions">
            <div className="bp-actions-row">
              <button
                className={`bp-btn bp-btn--primary ${isGeneratingBp?"bp-btn--generating":""} ${ollamaStatus!=="RUNNING"?"bp-btn--disabled":""}`}
                onClick={generateBlueprint}
                disabled={isGeneratingBp||ollamaStatus!=="RUNNING"}
              >
                {isGeneratingBp?"⟳ GENERATING...":"⬡ GENERATE PROJECT BLUEPRINT"}
              </button>
              <button
